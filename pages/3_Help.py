...
choose_func = st.radio('您对以下哪个功能有所疑问？',op_func,index=0)

if choose_func == op_func[0]:
    op_ques =['响应面拟合算法','上传数据样本格式']
    choose_ques = st.radio('您想了解哪部分内容？',op_ques,index=0)
    if choose_ques == op_ques[0]:
        ...
        choose_algorithm = st.radio('您想了解哪种算法的详细情况？',op_algorithm,index=0)
