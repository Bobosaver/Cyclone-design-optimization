...
def spline_interpolation():
    ...
    if file is not None:
        ...
        ax.set_xlabel('风速（m/s）',size=10,labelpad=5)
        ax.set_ylabel('粒径（μm）',size=10,labelpad=5)
        ax.set_zlabel('收集率（%）',size=10,labelpad=5)
        ax.set_title('旋风分离器拟合响应面',size=10) 
        ax.view_init(32, -32)
        st.pyplot(fig)
...
def collection_curve():
    ...
    if file is not None:
        ...
        plt.xlabel('粒径（μm）',fontsize=15)
        plt.ylabel('收集率（%）',fontsize=15)
        st.pyplot(fig)
...
img = mpimg.imread(r'C:\Users\Yin\Desktop\仿真结果记录\旋风分离器\Cyclone_Seperator_big2.jpg')
